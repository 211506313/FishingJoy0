...
bool GameScene::init()
{
	do
	{
		...
		int gold = FishingJoyData::getInstance()->getGold();
		...
		FishingJoyData::getInstance()->setMusic(true);
...
void GameScene::fishWillBeCaught(Fish* fish)
{
	...
	float fishPercents[	k_Fish_Type_Count] = { 1.1f, 1.1f, 1.1f, 1.1f ,1.0f, 1.0f, 1.0f, 0.9f, 0.9f, 0.9f, 0.8f, 0.8f, 0.7f, 0.7f, 0.6f, 0.6f, 0.5f, 0.5f };
	int cannonType = _cannonLayer->getWeapon()->getCannonType();
	int fishType = fish->getType();
	float percentage = weaponPercents[cannonType] * fishPercents[fishType];
	if(CCRANDOM_0_1() < percentage)
	{
		fish->beCaught();
		PersonalAudioEngine::sharedEngine()->playEffect("bgm_net.mp3");
		int reward = (fishType+2)*10;
...
void GameScene::alterGold(int delta)
{
	...
	_panelLayer->getGoldCounter()->setNumber(FishingJoyData::getInstance()->getGold());
}
...
void GameScene::resume()
{
	...
	FishingJoyData::getInstance()->getMusic() ? PersonalAudioEngine::sharedEngine()->resumeBackgroundMusic() : PersonalAudioEngine::sharedEngine()->pauseBackgroundMusic();
...
{
	PersonalAudioEngine::sharedEngine()->setBackgroundMusic(FishingJoyData::getInstance()->getMusic());
}

void GameScene::reset()
{
	FishingJoyData::getInstance()->reset();
